...
t_test <- function(data){
  ...
  t_test_result <- c()
  for (i in (1:n_rows)){
    t_test_result[i] <- t.test(group1[i,], group2[i,], alternative = "two.sided", mu = mean_diff[i], conf.level = alpha)$p.value
  }
...
wilcoxon_test <- function(data){
  ...
  for (i in (1:n_rows)){
    wilcoxon_test_result[i] <- wilcox.test(group1[i,], group2[i,], alternative = "two.sided", mu = 0, paired = FALSE)$p.value
